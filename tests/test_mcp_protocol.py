...
async def mcp_session():
    ...
    try:
        ...
        # Swallow "Attempted to exit cancel scope in a different task than it was entered in"
        # which is a known issue during teardown in some pytest environments
...
async def test_mcp_initialization(mcp_session: ClientSession):
    ...
    # Verify server info via the server object directly for in-memory tests
    assert mcp._mcp_server.name == "Vertex"

    # Verify capabilities via public API
...
async def test_call_simple_tool(mcp_session: ClientSession):
    ...
    result = await mcp_session.call_tool(
        "solve_linear_program",
        {
            ...
            "objective_coefficients": {"x": 3, "y": 2},
...
async def test_call_tool_with_validation_error(mcp_session: ClientSession):
    ...
    # Verify result indicates an error (FastMCP returns results with error flag set)
